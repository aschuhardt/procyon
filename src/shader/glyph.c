...
static glyph_vertex_bounds_t compute_glyph_vertex_bounds(glyph_shader_program_t* const shader) {
  ...
  if (bitmap_thin == NULL || bitmap_bold == NULL || shader->texture_w < 0 ||
    ...
    if (bitmap_size != 0) {
      ...
      if (combined_buffer != NULL) {
        ...
        log_error("Failed to allocate enough memory for combined glyph texture");
      }
    } else {
...
static void draw_glyph_batch(shader_program_t* const program, glyph_vertex_t* const vertices, GLushort* const indices, long glyph_count) {
    int buffer_size;

    // copy vertex data to video memory
    const int vertex_buffer_size = glyph_count * VERTICES_PER_GLYPH * sizeof(glyph_vertex_t);
    GL_CHECK(glBindBuffer(GL_ARRAY_BUFFER, program->vbo[VBO_GLYPH_POSITION]));
    ...
    // copy indices
    const int index_buffer_size = glyph_count * INDICES_PER_GLYPH * sizeof(GLushort);
    GL_CHECK(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, program->vbo[VBO_GLYPH_INDICES]));
...
glyph_shader_program_t* procy_create_glyph_shader(float scale) {
  ...
  glyph_shader->index_batch_buffer = malloc(sizeof(GLushort) * DRAW_BATCH_SIZE * INDICES_PER_GLYPH);
  glyph_shader->vertex_batch_buffer = malloc(sizeof(glyph_vertex_t) * DRAW_BATCH_SIZE * VERTICES_PER_GLYPH);
  ...
  if ((program->valid =
           procy_compile_vert_shader((char*)embed_glyph_vert, &program->vertex) &&
...
static void compute_glyph_vertices(glyph_shader_program_t* shader, glyph_vertex_bounds_t* const bounds, draw_op_t* const op, glyph_vertex_t* const vertices) {
  ...
  vertices[1] = (glyph_vertex_t){ x + bounds->width * bounds->scale, y, tx + bounds->tex_width, ty };
  vertices[2] = (glyph_vertex_t){ x, y + bounds->height * bounds->scale, tx, ty + bounds->tex_height };
  vertices[3] = (glyph_vertex_t){ x + bounds->width * bounds->scale, y + bounds->height * bounds->scale, tx + bounds->tex_width, ty + bounds->tex_height };
  for (int i = 0; i < 4; ++i) {
...
void procy_draw_glyph_shader(glyph_shader_program_t* shader, window_t* window) {
  ...
  // glyph vertex dimensions and u/v coordinates will be the same for every glyph drawn,
  // so compute them just once
  ...
  for (size_t i = 0; i < window->draw_ops.length; ++i) {
    ...
    memcpy(&index_batch[batch_index * INDICES_PER_GLYPH], temp_index_buffer, sizeof(temp_index_buffer));

    // if we've reached the end of the current batch, draw it and reset the index
  ...
  if (batch_index >= 0) {
    draw_glyph_batch(program, &vertex_batch[0], &index_batch[0], batch_index + 1);
  }
